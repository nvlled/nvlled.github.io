...
export const render: PageRender = async () => {
  return (
    <PostLayout data={data} icon="about">
      ...
      <a href="https://github.com/shikijs/shiki">shikijs</a>, but I couldn't get
      it to work, deno couldn't resolve the .wasm file or something. I instead
      went with <a href="https://github.com/speed-highlight/core">this</a>,
      which worked after a few fumbling around.
      <Post.BR />
...
type irctrl struct {
  ...
  nextStopped bool
      ...
      code. It took me a while to remember why the class is called Abstraction.
      I shortened AbstractionAction into Abstraction. What's AbstractAction
      action? I looked up the documentation, and see some terms like{" "}
      <em>JFC Action</em>, which I can only guess stands for{" "}
      <em>Jebus Fu**ing Chri** Action</em>. The horrors of CORBA and checked
      exceptions is coming back to me, so I'll try not to ponder what this code
      even does.
      {await codeBlock(
        "java",
        `
  class Abstraction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      <Post.BR />
      Then here's a lua, which isn't exceptionally terrible, I can actually tell
      at a glance what it does, but then it does a weird thing where it iterates
      the length of t three times until it finds a value != x.
      {await codeBlock(
        "lua",
        `
